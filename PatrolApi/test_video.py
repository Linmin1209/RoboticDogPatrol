...
class devClass:
    ...
    def SetSDKInitCfg(self):
        ...
        if sys_platform == 'windows':
            basePath = os.path.dirname(os.getcwd()).encode('gbk')
            strPath = basePath + b'\core'
            ...
            basePath = os.path.dirname(os.getcwd()).encode('utf-8')
            strPath = basePath + b'\core'
            sdk_ComPath = NET_DVR_LOCAL_SDK_PATH()
            sdk_ComPath.sPath = strPath
            if self.hikSDK.NET_DVR_SetSDKInitCfg(NET_SDK_INIT_CFG_TYPE.NET_SDK_INIT_CFG_SDK_PATH.value,
...
if __name__ == '__main__':
    ...
    dev.LoginDev(ip=b'192.168.1.64', username=b"admin", pwd=b"Robotdog1")  # 登录设备
