...
private:
    ...
    // ADD ↓↓↓
    // 一键返航相关
    nodeAttribute homeNode;     // 记录初始点
    bool        homeNodeSet = false; // 是否已设置返航点
    // ADD ↑↑↑
...
void slamDemo::addNodeAndEdge()
{
    ...
    if (!homeNodeSet && nodeAttributeList.size() == 1) {
    homeNode = nodeAttributeList[0];
    homeNodeSet = true;
    cout << "\033[1;32mHome node set at (" << homeNode.nodeX << ", " << homeNode.nodeY << ")\033[0m" << endl;
}
...
// ADD ↓↓↓
void slamDemo::navigateToHome()
{
    if (!homeNodeSet)
    {
        cout << "\033[1;31mHome node not set. Please start mapping first.\033[0m" << endl;
        return;
    }

    // 把当前位置作为临时节点
    nodeAttribute cur;
    cur.nodeName = ++node_name;
    cur.nodeX = currentOdom->pose().pose().position().x();
    cur.nodeY = currentOdom->pose().pose().position().y();
    cur.nodeZ = currentOdom->pose().pose().position().z();

    float sy = 2 * (currentOdom->pose().pose().orientation().w() * currentOdom->pose().pose().orientation().z() +
                    currentOdom->pose().pose().orientation().x() * currentOdom->pose().pose().orientation().y());
    float cy = 1 - 2 * (currentOdom->pose().pose().orientation().y() * currentOdom->pose().pose().orientation().y() +
                        currentOdom->pose().pose().orientation().z() * currentOdom->pose().pose().orientation().z());
    cur.nodeYaw = std::atan2(sy, cy);

    // 1. 添加当前节点
    nodeAttributeList.push_back(cur);

    // 2. 重新添加 homeNode（因为之前已经清空过）
    nodeAttribute tempHome = homeNode;
    nodeAttributeList.push_back(tempHome);

    // 3. 添加返航边
    edgeAttribute e;
    e.edgeName   = 9999;                      // 特殊编号
    e.edgeStart  = cur.nodeName;
    e.edgeEnd    = tempHome.nodeName;
    edgeAttributeList.push_back(e);

    // 4. 发布拓扑图并开始导航
    saveNodeAndEdge();
    defaultNavigation();  // 使用默认导航模式

    cout << "\033[1;32m[RETURN HOME] Navigating to initial point...\033[0m" << endl;
}
